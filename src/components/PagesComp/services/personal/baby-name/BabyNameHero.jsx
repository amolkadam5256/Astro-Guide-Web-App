...
const FeatureBlock = ({ icon, title, desc, point }) => (
  <div className="p-8 sm:p-10 bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-3xl group hover:border-primary transition-all">
    <div className="mb-6 group-hover:scale-110 transition-transform origin-left">
      {icon}
    </div>
    <h3 className="text-sm font-black uppercase tracking-widest text-dark dark:text-white mb-4">
      {title}
    </h3>
    <p className="text-xs sm:text-sm text-gray-500 leading-relaxed font-medium mb-6">
      {desc}
    </p>
    <div className="flex items-center space-x-2 text-[10px] font-black text-primary uppercase tracking-tighter">
      <Check size={12} />
      <span>{point}</span>
    </div>
  </div>
    ...
    <section className="py-20 bg-white dark:bg-[#050a15]">
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-16" data-aos="fade-down">
          ...
          <h1 className="text-3xl md:text-5xl font-black text-dark dark:text-white uppercase tracking-tighter mb-6">
            Baby Name <span className="text-primary">Numerology</span>
          </h1>
