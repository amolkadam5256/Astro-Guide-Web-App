    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
      {cards.map((card, i) => (
        <div
          ...
          className="p-8 bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-3xl group hover:border-primary transition-all"
        >
          ...
          <h3 className="text-sm font-black text-dark dark:text-white uppercase mb-4 tracking-widest">
            {card.title}
          </h3>
          <p className="text-[11px] sm:text-[12px] text-gray-500 font-medium leading-relaxed mb-6">
            {card.desc}
          </p>
          <div className="pt-4 border-t border-gray-100 dark:border-white/5 text-[9px] font-black uppercase text-primary">
            {card.detail}
          </div>
        </div>
      ))}
